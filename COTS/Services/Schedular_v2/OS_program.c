...
static TASK_t OS_TASKS[TASK_COUNT] = {NULL};
 ...
 void SCHED(void)
 {
	 ...
	 {
		 if(TASKS_INFO[TASK_Priority].first_time||(Tick_count-TASKS_INFO[TASK_Priority].last_run)==TASKS_INFO[TASK_Priority].Task_Periodicity)
		 {
			 // CALLS THE TASK ACCORDING TO THE PERIODICITY
			 
			 TASKS_INFO[TASK_Priority].TASK_ptr();
			 TASKS_INFO[TASK_Priority].last_run  = Tick_count;
			 TASKS_INFO[TASK_Priority].first_time  = 0;
		 }
	 }
